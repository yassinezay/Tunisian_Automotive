...
public function count(){
   return array_sum($_SESSION['panier']);
    ...
    public function editQuantite($quantite , $id_commande){
        $sql = "UPDATE ligne_commande  SET quantite = :qq WHERE id = :cid";
        $db = config::getConnexion();
        $req = $db->prepare($sql);
    ...
    public function getAllPanier()
    {
        $db = config::getConnexion();
        $DB = new config();
        $list = $DB->query('SELECT * FROM commande');
        return $list;
    }
    ...
    public function getAllPanierWithFacture($id)
    {
        ...
        $db = config::getConnexion();
        $DB = new config();
        $list = $DB->query("SELECT p.*, f.id as id_facture FROM `commande` p left join facture f on f.id_panier = p.id where p.id_user = '$id' ORDER BY id DESC");
        return $list;
    }
    ...
    public function getPanierProductById($idCmmande)
    {
        $db = config::getConnexion();
        $DB = new config();
        $list = $DB->query('SELECT * FROM ligne_commande lp inner join articles v on lp.id_produit = v.id where lp.id_commande = '.$idCmmande);
        return $list;
    }
    ...
    public function getLastPanier($id_u)
    {
      ...
      $DB = new config();
        ...
        $lastPanier = $DB->query("SELECT * FROM commande c WHERE c.paid=0 and c.id_user= '$id_u' ORDER BY id DESC LIMIT 1");
        $id_panier;
        if (!empty($lastPanier)) {
            $id_panier = $lastPanier[0]->id;
        } else {
            $sql = "INSERT INTO commande  (paid,id_user)
                VALUES (:paid,:id_user)";
    ...
    public function getLastFacture()
    {
      ...
      $DB = new config();
        ...
        $lastFactureanier = $DB->query('SELECT * FROM facture ORDER BY id DESC LIMIT 1');
    ...
    public function getPanierProduct($id_u)
    {
        $db = config::getConnexion();
        $DB = new config();
        $list = $DB->query('SELECT * FROM ligne_commande lp inner join articles v on lp.id_produit = v.id where lp.id_commande = '.$this->getLastPanier($id_u));
        return $list;
    }
    ...
    public function editPanier($id_panier)
    {
        $db = config::getConnexion();
        $DB = new config();
        $sql = "UPDATE commande set paid = 1 WHERE id = :panier";
    ...
    public function createFacture($id){
        $db = config::getConnexion();
        $DB = new config();
            $sql = "INSERT INTO facture (id_panier)
            VALUES (:id_panier)";
            ...
            header('Location: ../View/facture.php?id_facture='.$this->getLastFacture());
    ...
    public function getFacture($id_facture){
        $db = config::getConnexion();
        $DB = new config();
        $facture = $DB->query('SELECT * FROM facture where id = '.$id_facture);
        return $facture[0];
    }
    ...
    public function getFactureProducts($id_facture)
    {
        $db = config::getConnexion();
        $DB = new config();
        $facure = $DB->query('SELECT * FROM facture where id = '.$id_facture);
        $list = $DB->query('SELECT * FROM ligne_commande lp inner join articles v on lp.id_produit = v.id where lp.id_commande = '.$facure[0]->id_panier);
        return $list;
    }
    ...
    public function getTotalFacture($id_facture)
    {
        ...
        foreach ($this->getFactureProducts($id_facture) as $value) {
            $total += $value->price * $value->quantite ; 
          }
         return $total;
    }
