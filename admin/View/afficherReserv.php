...
$sql = 'SELECT COUNT(*) AS ID FROM `voitures`;';
...
$sql = 'SELECT * FROM `voitures` ORDER BY `ID` DESC LIMIT :premier, :parpage;';
    ...
    <div id="main-wrapper" data-layout="vertical" data-navbarbg="skin5" data-sidebartype="full"
            ...
            <div class="page-breadcrumb bg-white">
                <div class="row align-items-center">
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                        <h4 class="page-title">Liste des reservations</h4>
                    </div>
                </div>
                <!-- /.col-lg-12 -->
            </div>
        ...
        <?php
            ...
            ?>
             ...
             if(require("../../db.php")) {
    $req =$db->query("SELECT voitures.ID as ID,voitures.Marque as Marque ,voitures.PrixJour as prix, voitures.Matricule as Matricule ,voitures.Modele as Modele ,voitures.Img as Img ,reservation.date as datereserv ,reservation.idreserv as idreserv FROM
    voitures RIGHT JOIN reservation ON voitures.ID= reservation.idvt");
...
<td><?= $donnee['ID'] ?></td>
                ...
                <td><?= $donnee['prix'] ?> D</td>
                <td>
                <img src="<?= $donnee['Img']; ?>" style="width: 70%">
                </td>
                <td><?= $donnee['idreserv'] ?></td>
                    <td><?= $donnee['datereserv'] ?></td>
                    



                    <td align="center">
                        <form method="POST" action="editerReserv.php">
                            <input type="submit" name="update" value="Update" class="btn">
                            <input type="hidden" value=<?PHP echo $donnee['idreserv']; ?> name="idreserv">
                        </form>
                    </td>
                    
                    <td>
                        <a href="supprimerReserv.php ? idreserv=<?php echo $donnee['idreserv']; ?>">Delete</a>
                    </td>
</tr>
         ...
         <nav>
                    <ul class="pagination">
                        ...
                        <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
                            <a href="afficherReserv.php?page=<?= $currentPage - 1 ?>" class="page-link">Précédente</a>
                        </li>
                        <?php for($page = 1; $page <= $pages; $page++): ?>
                          <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
                          <li class="page-item <?= ($currentPage == $page) ? "active" : "" ?>">
                                <a href="afficherReserv.php?page=<?= $page ?>" class="page-link"><?= $page ?></a>
                            </li>
                        <?php endfor ?>
                          <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
                          <li class="page-item <?= ($currentPage == $pages) ? "disabled" : "" ?>">
                            <a href="afficherReserv.php?page=<?= $currentPage + 1 ?>" class="page-link">Suivante</a>
                        </li>
                    </ul>
                </nav>
                        </div>
