...
if(isset($_SESSION['email']) and isset($_SESSION['password']))
{
  ...
  if(isset($_POST['send']))
  {
    ...
    $date=date('Y-m-d');
    $heure=date('H:i');
    $req1=$db->query("insert into reply values (NULL,'$msg','$date','$heure','$id','$x')"); 
    header("Refresh:0");
  }
  //pagination
...
//forum counter
  $sqll="update visitorc set forum_visitor=forum_visitor+1";
...
while($data=$res->fetch(PDO::FETCH_ASSOC))
{
  ...
  echo $data['nom_user'];
  echo '&nbsp;'.$data['prenom_user'].'</div>';
   echo '<div id="div2">'.$data['texte_message'];
  echo '<br> Posté le : '.changedate($data['date_message']);
  echo ' à '.$data['heure_message'];
  echo '<br><form action="" method="post" name="reply">
  <textarea name="replymessage"   placeholder="Votre message" id="c"></textarea>
  <textarea name="id_message"   placeholder="Votre message" id="c" hidden>'.$data['id_message'].'</textarea>
  <p style="color: red" id="textEr"></p>
  <input type="submit" name="send" value="Reply" id="reply" class="btn2"  onclick="Showuptextarea()">
  </form>';
...
<nav>
              <ul class="pagination pagination-lg justify-content-center">
                ...
                <?php if($currentPage > 1) : ?>
                  <a class="page-link" href="forum.php?page=<?= $currentPage - 1 ?>" aria-label="Previous">
                ...
                <?php if($currentPage < $pages) : ?>
                  <a class="page-link" href="forum.php?page=<?= $currentPage + 1 ?>" aria-label="Next">
